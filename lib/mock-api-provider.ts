 ...
 *   mock.addPR("acme/app", { number: 1, headRefName: "cursor/fix", ... });
 *   setProvider(mock);
 *   // ... exercise app code ...
 *   assert.equal(mock.prComments.get("acme/app:1")!.length, 1);
 *   mock.reset();           // wipe everything for the next test
 *   resetProvider();         // restore real provider
 */
...
export class MockApiProvider implements ApiProvider {
  ...
  // ── Status cache invalidation counter ──
  ...
  ensureGh(): void {
    // Always succeeds in mock — gh is "installed" and "authenticated".
  }
  ...
  listOpenPRs(repo: string): PR[] {
    return (this.prs.get(repo) ?? []).filter((pr) => (pr.state ?? "open") === "open");
  }

  async listOpenPRsAsync(repo: string): Promise<PR[]> {
  ...
  // ── gh passthrough ──
  // The mock interprets common gh subcommands to manipulate state, so that
  // code using ghQuietAsync for PR mutations (merge, edit, review, etc.)
  // works correctly against the mock.
  ...
  private _handleGhCall(args: string[]): string {
    ...
    if (args[0] === "pr" && args[1] === "view") {
      return this._handlePrView(args);
    }
    // ── pr edit ──
    if (args[0] === "pr" && args[1] === "edit") {
      return this._handlePrEdit(args);
    }
    // ── pr merge ──
    if (args[0] === "pr" && args[1] === "merge") {
      return this._handlePrMerge(args);
    }
    // ── pr ready ──
    if (args[0] === "pr" && args[1] === "ready") {
      return this._handlePrReady(args);
    }
    // ── pr review ──
    if (args[0] === "pr" && args[1] === "review") {
      return this._handlePrReview(args);
    }
    // ── pr close ──
    if (args[0] === "pr" && args[1] === "close") {
      return this._handlePrClose(args);
    }
    // ── pr comment ──
    if (args[0] === "pr" && args[1] === "comment") {
      return this._handlePrComment(args);
    }
    // ── pr create ──
    if (args[0] === "pr" && args[1] === "create") {
      return this._handlePrCreate(args);
    }
    // ── pr list ──
    if (args[0] === "pr" && args[1] === "list") {
      return this._handlePrList(args);
    }
    // ── run list ──
    if (args[0] === "run" && args[1] === "list") {
      return this._handleRunList(args);
    }
    // ── run rerun ──
    if (args[0] === "run" && args[1] === "rerun") {
      return this._handleRunRerun(args);
    }
    // ── issue create ──
    if (args[0] === "issue" && args[1] === "create") {
      return this._handleIssueCreate(args);
    }
    // ── issue comment ──
    if (args[0] === "issue" && args[1] === "comment") {
      return "";
    }
    // ── api repos/{repo} (repo metadata) ──
    if (args[0] === "api" && args[1]?.startsWith("repos/")) {
      return this._handleApiRepos(args);
    }
    // ── api user ──
    if (args[0] === "api" && args[1] === "user") {
      return JSON.stringify({ login: this.currentUser });
    }
    // ── api graphql ──
    if (args[0] === "api" && args[1] === "graphql") {
      return this._handleApiGraphql(args);
    }
  ...
  private _handlePrView(args: string[]): string {
    ...
    if (jsonFlag === "isDraft" && jqFlag === ".isDraft") {
      return `${pr.isDraft ?? false}\n`;
    }
    if (jsonFlag === "baseRefName" && jqFlag === ".baseRefName") {
  ...
  private _handlePrMerge(args: string[]): string {
    const repo = this._findRepo(args) ?? "";
    // PR number can be at various positions; find the numeric arg.
  ...
  private _handleRunRerun(args: string[]): string {
    ...
    // Find and reset the run's conclusion.
  ...
  private _handleApiRepos(args: string[]): string {
    ...
    // repos/{owner}/{name}/merges (merge base into branch)
  ...
  private _handleApiGraphql(args: string[]): string {
    // Return minimal valid responses for common GraphQL queries.
    ...
    // Co-author query (commit authors).
  ...
  async cursorGetArtifactDownloadUrl(_apiKey: string, agentId: string, absolutePath: string): Promise<{ url: string; expiresAt?: string }> {
  ...
  loadConfig(): Copserc | null {
    return this.config;
  }

  getConfiguredRepos(): string[] | null {
