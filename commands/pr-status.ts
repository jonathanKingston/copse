...
function parseArgs(argv: string[]): PrStatusParsed {
...
export const prStatusCommand: Command = {
  ...
  run(args: string[]): void {
    const { repo, agent, mineOnly } = parseArgs(args);
...
const isDirectExecution =
  process.argv[1] &&
  (process.argv[1].endsWith("/pr-status.js") || process.argv[1].endsWith("/pr-status.ts"));

if (isDirectExecution) {
  try {
    const args = process.argv.slice(2);
    if (prStatusCommand.validate) prStatusCommand.validate(args);
    prStatusCommand.run(args);
  } catch (e: unknown) {
